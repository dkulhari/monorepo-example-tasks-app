...
class MigrationRunner {
    ...
    async up() {
        ...
        const pendingMigrations = migrationFiles.filter(m => !appliedMigrations.has(m.filename));
    ...
    async status() {
        ...
        const appliedMap = new Map(appliedRecords.map(r => [r.filename, r]));
        ...
        for (const file of migrationFiles) {
            ...
            const appliedAt = applied
                ...
                : "-";
            console.log(`${file.filename.padEnd(40)} | ${status.padEnd(10)} | ${appliedAt}`);
        }
