...
export function tenantMiddleware() {
    return createMiddleware(async (c, next) => {
        ...
        const membership = await db.query.userTenantAssociations.findFirst({
            where: and(eq(userTenantAssociations.tenantId, tenant.id), eq(userTenantAssociations.userId, user.sub), eq(userTenantAssociations.status, "active")),
        });
        if (!membership) {
...
function getTenantIdentifier(c) {
...
export function getTenant(c) {
    const tenant = c.get("tenant");
    return tenant;
}
export function getUserRole(c) {
